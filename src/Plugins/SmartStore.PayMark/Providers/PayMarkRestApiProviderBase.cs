...
{
    public abstract class PayMarkRestApiProviderBase<TSetting> : PaymentMethodBase, IConfigurable where TSetting : PayMarkApiSettingsBase, ISettings, new()
    {
        private readonly string _providerSystemName;

        protected PayMarkRestApiProviderBase(string providerSystemName)
		{
        ...
        public IPayMarkService PayMarkService { get; set; }
        ...
        public override ProcessPaymentResult ProcessPayment(ProcessPaymentRequest processPaymentRequest)
		{
			...
			HttpContext.Session.SafeRemove("PayMarkCheckoutCompleted");

            var storeId = processPaymentRequest.StoreId;
            var customer = Services.WorkContext.CurrentCustomer;
            var session = HttpContext.GetPayMarkState(_providerSystemName, customer, storeId, GenericAttributeService);
			...
			if (session.AccessToken.IsEmpty() || session.PaymentId.IsEmpty())
			{
                ...
                result.AddError(T("Plugins.SmartStore.PayMark.SessionExpired"));
            ...
            var apiResult = PayMarkService.ExecutePayment(settings, session);
			...
			if (apiResult.Success && apiResult.Json != null)
			{
				...
				if (!state.IsCaseInsensitiveEqual("failed"))
				{
					...
					if (intent.IsCaseInsensitiveEqual("sale"))
					{
						...
						session.PaymentInstruction = PayMarkService.ParsePaymentInstruction(apiResult.Json.payment_instruction) as PayMarkPaymentInstruction;

						// Test session data:
						//session.PaymentInstruction = new PayMarkPaymentInstruction
						//{
						...
						//	RecipientBanking = new PayMarkPaymentInstruction.RecipientBankingInstruction
						//	{
					...
					if (relatedObject != null)
					{
						...
						result.NewPaymentStatus = PayMarkService.GetPaymentStatus(state, reasonCode, PaymentStatus.Authorized);
				...
				else
				{
					...
					result.Errors.Add(T("Plugins.SmartStore.PayMark.PaymentExecuteFailed").Text.Grow(failureReason, " "));
				}
			}
		...
		public override void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
		{
            ...
            var session = HttpContext.GetPayMarkState(_providerSystemName, customer, order.StoreId, GenericAttributeService);
            var instruction = PayMarkService.CreatePaymentInstruction(session.PaymentInstruction);

			if (instruction.HasValue())
			{
				HttpContext.Session["PayMarkCheckoutCompleted"] = instruction;
		...
		public override CapturePaymentResult Capture(CapturePaymentRequest capturePaymentRequest)
        {
            ...
            var session = new PayMarkSessionData { ProviderSystemName = _providerSystemName };

            var apiResult = PayMarkService.EnsureAccessToken(session, settings);
			if (apiResult.Success)
			{
				apiResult = PayMarkService.Capture(settings, session, capturePaymentRequest);
        ...
        public override RefundPaymentResult Refund(RefundPaymentRequest refundPaymentRequest)
        {
			...
			if (apiResult.Success)
			{
				apiResult = PayMarkService.Refund(settings, session, refundPaymentRequest);
				if (apiResult.Success)
				{
        ...
        public override VoidPaymentResult Void(VoidPaymentRequest voidPaymentRequest)
        {
			...
			if (apiResult.Success)
			{
				apiResult = PayMarkService.Void(settings, session, voidPaymentRequest);
				if (apiResult.Success)
				{

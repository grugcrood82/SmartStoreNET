...
using SmartStore.PayMark.PayMarkSvc;
...
{
	public abstract class PayMarkProviderBase<TSetting> : PaymentMethodBase, IConfigurable where TSetting : PayMarkApiSettingsBase, ISettings, new()
    {
        protected PayMarkProviderBase()
		{
		...
		protected PayMarkAPIAASoapBinding GetApiAaService(TSetting settings)
		{
			var service = new PayMarkAPIAASoapBinding();

			service.Url = settings.UseSandbox ? "https://api-3t.sandbox.PayMark.com/2.0/" : "https://api-3t.PayMark.com/2.0/";
		...
		protected PayMarkAPISoapBinding GetApiService(TSetting settings)
		{
			var service = new PayMarkAPISoapBinding();
		...
		protected CustomSecurityHeaderType GetApiCredentials(PayMarkApiSettingsBase settings)
		{
        ...
        public override RefundPaymentResult Refund(RefundPaymentRequest request)
        {
			...
			// merchant must accept the payment in his PayMark account
			...
			if (request.IsPartialRefund)
			{
				...
				// see https://developer.PayMark.com/docs/classic/express-checkout/digital-goods/ECDGIssuingRefunds/
				// https://developer.PayMark.com/docs/classic/api/merchant/RefundTransaction_API_Operation_NVP/
				var memo = Services.Localization.GetResource("Plugins.SmartStore.PayMark.PartialRefundMemo", 0, false, "", true);
				if (memo.HasValue())
				{
